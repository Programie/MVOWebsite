...
if ($calendar == "internal.ics" or $calendar == "public.ics")
{
	if ($calendar == "internal.ics")
	{
		if (Constants::$pagePath[2])
		{
			...
			$query->execute(array(":calendarToken" => Constants::$pagePath[2]));
			$row = $query->fetch();
			if ($row->id)
			{
	...
	foreach ($dates as $date)
	{
		...
		$event->setUID($date->id . "-" . $date->startDate . "@dates." . $_SERVER["SERVER_NAME"]);
		$calendar->add($event);
	}
...
if (substr(Constants::$pagePath[1], -4) == ".pdf")
{
	...
	if ($dates)
	{
		...
		$header = array(array("width" => $contentWidth * 0.15, "text" => "Datum"), array("width" => $contentWidth * 0.15, "text" => "Zeit"), array("width" => $contentWidth * 0.4, "text" => "Veranstaltung"), array("width" => $contentWidth * 0.3, "text" => "Ort"));
		...
		foreach ($header as $column => $cell)
		{
			...
			$headerCells[] = array("width" => $cell["width"], "height" => 10, "text" => $cell["text"], "align" => "C", "lineArea" => $lineArea);
		}

		$callbackData = array("groupTitles" => $groupTitles, "tableHeaders" => array($headerCells), "title" => $title);
		...
		foreach ($dates as $row)
		{
			...
			$row = array(Dates::getDateText($row->startDate), Dates::getTimeText($row->startDate, $row->endDate), utf8_decode(implode("\n", $description)), utf8_decode($row->location->name));
			...
			foreach ($row as $column => $cell)
			{
				...
				$cells[] = array("width" => $header[$column]["width"], "height" => 7, "text" => $cell, "align" => "L", "lineArea" => $lineArea);
			}
			$data[] = $cells;
		}
